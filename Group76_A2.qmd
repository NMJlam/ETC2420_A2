...
-   Explain what the box-whisker and the bar graph shows about the foot traffic in each of the locations

### Conclusions

-   Conclusions on the central tendency in a non-technical way.

## Requirement 2: Appropriate Summary Statistics

### Approach

Explain why we have continuous and not discrete values – why?

This will guide the summary statistics we will provide.
...
Explain the statistics in a technical way

### Conclusions

Explain the statistics in a non-technical way
...
-   Explain why we reported standard error

-   Explain why we have the parameters
...
```{r fit-model}
fit_flinders <- fitdistr(flinders_street_traffic, "normal")
fit_southern <- fitdistr(southern_cross_traffic, "normal")
fit_qv       <- fitdistr(qv_melbourne_traffic, "gamma")

```

### Results

MUST SEPARATE GAMMA AND NORMAL

```{r}
estimates <- tibble(
  location = c("Flinders Street", "Southern Cross", "QV Melbourne"),
  model    = c("Normal", "Normal", "Gamma"),
  par1     = c(fit_flinders$estimate["mean"],
               fit_southern$estimate["mean"],
               fit_qv$estimate["shape"]),
  se1      = c(fit_flinders$sd["mean"],
               fit_southern$sd["mean"],
               fit_qv$sd["shape"]),
  par2     = c(fit_flinders$estimate["sd"],
               fit_southern$estimate["sd"],
               fit_qv$estimate["rate"]),
  se2      = c(fit_flinders$sd["sd"],
               fit_southern$sd["sd"],
               fit_qv$sd["rate"])
)
...
Explain why plotting using a QQ plot allows us to assess the model fits
...
Interpret the results by observing the variances from the line

### Interpretation

Show through a separate plot how they are similar and explain why using the QQ plot

### Conclusions

Concluded in a non-technical way

# Task 2: \[20 marks\]

## Introduction

The traffic regulations require that crossings are designed to have smooth pedestrian flow 90% of the time. To help the engineering team assess whether the crossings are large enough to meet the regulations, they would like an estimate of 90th percentile amount of traffic for each crossing. Use two approaches (as outlined below) and provide interval estimates for each.

Required analyses and components:

-   As the first approach, use the *sample quantile* as the estimator. This is implemented in R by the function `quantile()`.

-   As the second approach, use estimators based on the models you fitted in [Task 1](file:///Users/natha/Downloads/assignment2.html#task-1).
...
The engineers have identified that they need to make some upgrades to the crossings outside of the two railway stations (Flinders Street and Southern Cross). To save money, they are hoping they can use the same design and materials for both crossings. They can do so only if the average number of people using each crossing is similar. Specifically, the difference between the averages needs to be no more than 80 people per hour.

Can the engineers safely save money in this case? Is the difference small enough and how certain can we be?

Required analyses and components:
-   Calculate an appropriate 95% confidence interval for the difference in the mean number of people crossing outside the two railway stations. x
-   Comment on whether it is valid to use the Central Limit Theorem when calculating this confidence interval. x 
-   Based on your confidence interval, what conclusions can you provide to the engineering team? x
-   Illustrate your analysis with any appropriate plots.

## Methodology

- Determine if the averages of the 2 crossings (flinders stree and southern cross) are greater than 80 people per hour
...
# Your 95 % CI for (Flinders − Southern Cross)
ci_lower <- -88.59
ci_upper <- -8.92
...
#ci_res$estimate
ci_res$conf.int
# ^^ 95% confident that we the mean difference lies between -88.58 and -8.91 -> does not lie within the tolerance
```
