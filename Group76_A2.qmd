...
The uncertainty in these estimates is then assessed using a non-parametric bootstrap of the observed data.

The procedure is as follows: 1. Fit the model: Estimate the model parameters (mean and standard deviation for the Normal; shape and rate for the Gamma) from each data set using MASS::fitdistr().

```         
2.  Resample the data: Draw bootstrap samples from the observed counts with replacement, keeping the same sample size as the original data.

3.  Re-fit and compute: For each bootstrap sample, re-fit the same model and calculate the 90th percentile using the updated parameter estimates (qnorm() or qgamma()).

4.  Repeat and collate: Repeat this process 5 000 times to produce a bootstrap distribution of 90th-percentile estimates.

5.  Construct a confidence interval: Form the 95 % confidence interval by taking the 2.5 th and 97.5 th percentiles of the bootstrap distribution (the percentile bootstrap method).
```

The parametric bootstrap method leverages the assumed distributional structure of the data. If the fitted parametric model (Normal or Gamma) accurately represents the true population distribution—as validated by the goodness-of-fit diagnostics in Task 1—this approach can provide several advantages:

1.  **Smoothing over sampling irregularities**: The parametric model smooths over random fluctuations and irregularities in the observed sample, potentially leading to more stable and precise estimates when the model assumption holds.

2.  **Validation through comparison**: Comparing parametric and non-parametric bootstrap results serves a dual purpose: if the methods yield similar confidence intervals, this provides evidence that (a) the parametric model assumption is reasonable, and (b) the quantile estimates are robust to methodological choices.
...
```{r}
# ==============================================================================
# Parametric Bootstrap Method - Combined Function
# ==============================================================================

find_90thPercentile_model <- function(data, model = "normal", trials = 5000) {
  # Remove missing/non-finite values
  x <- as.numeric(data[is.finite(data)])
  n <- length(x)
  ...
  if (model == "normal") {
    ...
    # Fit Normal distribution to original data (MLE)
    mu_hat <- mean(x)
    sigma_hat <- sd(x)
    
    # Parametric bootstrap: Generate from fitted Normal distribution
    q_boot <- replicate(trials, {
      # Generate NEW data from Normal(μ̂, σ̂)
      xb <- rnorm(n, mean = mu_hat, sd = sigma_hat)
      # Calculate empirical 90th percentile from simulated data
      quantile(xb, probs = 0.90, type = 7)
    })
    ...
    # Fit Gamma distribution to original data (MLE)
    fit <- MASS::fitdistr(x, densfun = "gamma")
    shape_hat <- unname(fit$estimate["shape"])
    rate_hat  <- unname(fit$estimate["rate"])
    
    # Parametric bootstrap: Generate from fitted Gamma distribution
    q_boot <- replicate(trials, {
      # Generate NEW data from Gamma(α̂, β̂)
  ...
  # Construct 95% CI using percentile bootstrap method
  ci <- quantile(q_boot, c(0.025, 0.975), names = FALSE)
  
  # Return results
  list(
    q90_estimate = mean(q_boot),
    ci_lower = ci[1],
    ci_upper = ci[2]
  )
}
...
flinders_param <- find_90thPercentile_model(flinders_street_traffic, model = "normal")
southern_param <- find_90thPercentile_model(southern_cross_traffic, model = "normal")
qv_param       <- find_90thPercentile_model(qv_melbourne_traffic, model = "gamma")
```

```{r}
# --- Point estimates (non-bootstrapped) ---
