...
#### Justification for Comparing Two Methods

Because the task notes there’s “more than one way of doing this,” we report results from two taught methods to check robustness. The Bayesian Beta–Binomial approach (Weeks 7–8) treats each day as a yes/no outcome, uses a simple prior from the notes, and returns a clear uncertainty range for the exceedance rate that we then convert directly to revenue. The frequentist bootstrap (Week 5) stays fully data-driven by repeatedly resampling the daily outcomes to build an empirical uncertainty range, which we also map to revenue. Both methods rely on the same course assumptions (independent days with a stable exceedance rate at each location) and produce very similar rankings and ranges, so our recommendation does not depend on the modelling framework. Using both therefore strengthens confidence in the final billboard choice.
